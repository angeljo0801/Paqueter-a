...
class StoreOcrParser {
  ...
  static StoreOcrParse parse(String rawText) {
    ...
    if (store == 'Amazon') {
      ...
      if (total == 0) {
        // Amazon mobile checkout may place the amount several OCR lines away
        // from the visual Total label. Restrict fallback to the checkout area
        // so product prices and ratings are not treated as the order total.
