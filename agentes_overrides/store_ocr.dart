    ...
    if (store == 'Amazon') {
      ...
      total = _labeledAmount(
        ...
        [
          ...
          RegExp(r'^\s*TOTAL\s*[:\-]?\s*    } else if (store == 'AliExpress') {
      subtotal = _labeledAmount(
        lines,
...
, caseSensitive: false),
      ...
      if (total == 0) {
        // Amazon's mobile checkout often renders "Total" and the amount in
        // separate OCR blocks. The amount can end up several lines away from
        // the label even though it is visually adjacent. Limit the fallback
        // to the checkout footer so ratings/product prices are not mistaken
        // for the order total.
        total = _checkoutFooterAmount(
          lines,
